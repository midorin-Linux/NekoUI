...
use crate::{
    ...
    store::MemoryEntry,
    vector_db::{SearchFilter, VectorDbClient},
};
...
impl MidTermMemory {
    ...
    pub async fn store_summary(
        ...
        payload.insert(
            ...
            json!(session_key.conversation_id.to_string()),
        );
        payload.insert("user_id".to_string(), json!(session_key.user_id));
        ...
        debug!(id = %id, session = %session_key.conversation_id, "stored mid-term summary");
        Ok(())
    }
    ...
    pub async fn search_with_embedding(
        ...
        let filter = SearchFilter {
            must: vec![crate::vector_db::FilterCondition::Match {
                key: "conversation_id".to_string(),
                value: json!(session_key.conversation_id.to_string()),
            }],
            should: vec![],
        };
    ...
    pub async fn delete_old_entries(&self) -> Result<u64> {
        ...
        let filter = SearchFilter {
            must: vec![crate::vector_db::FilterCondition::Range {
