...
use nekoui_config::loader::Config as AppConfig;
...
impl MemoryStore {
    ...
    pub async fn promote_to_mid_term(
        ...
        debug!(session = %session_key.conversation_id, "promoted short-term to mid-term");
        Ok(())
    }
    ...
    pub async fn extract_long_term(
        ...
        debug!(session = %session_key.conversation_id, fact_count = fact_count, "extracted long-term facts");
        Ok(())
    }
