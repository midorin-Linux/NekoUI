...
use nekoui_config::loader::{Config, DEFAULT_QDRANT_URL};
...
/// Existing values take priority over new values  Ethis prevents accidentally
...
fn merge_with_existing(new_config: &Config) -> Result<Config> {
    ...
    merge_server(&mut merged, &existing);
...
/// Keep the existing server settings if they are not placeholders.
fn merge_server(merged: &mut Config, existing: &Config) {
    if !existing.server.bind_address.is_empty() && existing.server.bind_address != "0.0.0.0:3000" {
        merged.server.bind_address = existing.server.bind_address.clone();
    }
    if existing.server.auth_token.is_some() {
        merged.server.auth_token = existing.server.auth_token.clone();
    }
}
...
fn merge_parameters(
    merged_params: &mut nekoui_config::loader::Parameters,
    existing: &nekoui_config::loader::Parameters,
) {
