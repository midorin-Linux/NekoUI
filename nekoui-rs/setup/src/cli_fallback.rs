use nekoui_config::loader::{
    Config, ConversationModel, EmbeddingModel, Memory, Parameters, Provider, SecretKey,
    ServerConfig, SummarizerModel, ToolPermissions, VectorDb,
};
use tracing::warn;
...
fn resolve_provider(provider: &str, model: &str, base_url: &str) -> (String, String) {
    ...
    let (url, default_model) = match provider.to_ascii_lowercase().as_str() {
        ...
        _ => {
            warn!(provider = %provider, "unknown AI provider name  Efalling back to OpenAI defaults");
            let model = if model.is_empty() { "gpt-4o" } else { model };
