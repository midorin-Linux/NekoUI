...
use dashmap::DashMap;
use nekoui_config::loader::{Config, Parameters};
use nekoui_domain::agent::{
...
impl AgentRuntime {
    ...
    pub async fn submit(
        ...
        let caller_context = CallerContext {
            ...
            conversation_id: Some(session_key.conversation_id.to_string()),
        };
            ...
            && self.summarizing.insert(session_key.clone(), ()).is_none()
        {
            ...
            tokio::spawn(async move {
                if let Err(error) = this
                    ...
                    .await
                {
                    warn!(session = %key.conversation_id, error = %error, "background promote failed");
                } else {
