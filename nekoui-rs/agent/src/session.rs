...
impl SessionManager {
    ...
    pub fn clear(&self, session_key: &SessionKey) -> Result<()> {
        if self.sessions.remove(session_key).is_some() {
            debug!(session = %session_key.conversation_id, "session cleared");
        } else {
    ...
    pub fn get_or_create(&self, session_key: &SessionKey) -> Arc<Mutex<Session>> {
        self.sessions
            ...
            .or_insert_with(|| {
                debug!(session = %session_key.conversation_id, "created new session");
                let now = Utc::now();
    ...
    pub fn get(&self, session_key: &SessionKey) -> Result<Arc<Mutex<Session>>> {
        self.sessions
            .get(session_key)
            .map(|entry| {
                debug!(session = %session_key.conversation_id, "found existing session");
                entry.value().clone()
            })
            .ok_or_else(|| {
                debug!(target_session = %session_key.conversation_id, "non-existent session");
