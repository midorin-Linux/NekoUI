...
/// HTTP/WebSocket サーバー設定
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ServerConfig {
    /// バインドアドレス（デフォルト: 0.0.0.0:3000）
    #[serde(default = "default_bind_address")]
    pub bind_address: String,

    /// CORS 許可オリジン一覧（空なら localhost のみ）
    #[serde(default)]
    pub allowed_origins: Vec<String>,

    /// Bearer トークン認証（None なら認証なし）
    pub auth_token: Option<SecretKey>,

    /// WebSocket ping 間隔（秒、デフォルト: 30）
