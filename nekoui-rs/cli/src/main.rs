...
use nekoui_agent::runtime::{AgentRuntime, RuntimeInitProgress};
            ...
            Command::new("start")
                ...
                .arg(
                    clap::Arg::new("api-key")
                        .long("api-key")
                        .help("API key for the AI provider (used with --skip-setup). Prefer the nekoui_API_KEY environment variable instead to avoid exposing secrets in process listings.")
                        .num_args(1),
                )
                .arg(
...
async fn run() -> Result<()> {
    ...
    match matches.subcommand() {
        Some(("start", sub_matches)) => {
            ...
            agent_init_bar.finish_and_clear();
            println!("    {} Agent runtime initialized", "[ok]".green());
