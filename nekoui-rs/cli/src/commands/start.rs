...
use nekoui_infra::logging::{WorkerGuard, init_tracing};
...
impl StartCommand {
    ...
    pub async fn start(sub_matches: &ArgMatches) -> Result<(Config, WorkerGuard, MemoryStore)> {
        ...
        println!("    {} Tracing initialized", "[ok]".green());
        ...
        let config = if config_exists() {
            ...
            Config::load()
                ...
                .inspect(|_| {
                    ...
                    println!("    {} Configuration loaded", "[ok]".green());
                })?
        } else {
            spinner.finish_and_clear();
            ...
            if skip_setup || has_env_token() {
                ...
                if has_env_token() {
                    info!("nekoui_API_KEY environment variable found, skipping setup wizard");
                    println!(
                        "    {} nekoui_API_KEY detected, using environment",
                        "i".cyan()
                    );
                    ...
                    if let Some(cfg) = config_from_env() {
                        ...
                        bail!("failed to build config from nekoui_API_KEY");
                    }
                } else {
                    ...
                    let api_key = sub_matches
                        ...
                        .cloned()
                        .or_else(|| std::env::var("nekoui_API_KEY").ok())
                        .unwrap_or_default();

                    if api_key.is_empty() {
                        bail!("--api-key or nekoui_API_KEY is required when using --skip-setup");
                    }

                    let provider = sub_matches
                        .get_one::<String>("provider")
                        .cloned()
                        .or_else(|| std::env::var("nekoui_PROVIDER").ok())
                        .unwrap_or_default();

                    let model = sub_matches
                        .get_one::<String>("model")
                        .cloned()
                        .or_else(|| std::env::var("nekoui_MODEL").ok())
                        .unwrap_or_default();

                    let base_url = sub_matches
                        .get_one::<String>("base-url")
                        .cloned()
                        .or_else(|| std::env::var("nekoui_BASE_URL").ok())
                        .unwrap_or_default();

                    let web_search = sub_matches.get_flag("web-search");

                    let cfg = nekoui_setup::cli_fallback::make_config(
                        &api_key, &provider, &model, &base_url, web_search,
                    );
                    info!(
        ...
        println!("    {} context memory initialized", "[ok]".green());
