...
`nekoui` は Rust 製の Discord 用 AI エージェントです。Rig SDK を基盤に、Discord コマンド、3 層メモリ、OpenAI 互換モデル設定、セットアップウィザードを備えています。
...
- ログは `logs/nekoui.log` に日次ローテーションで出力します

## リポジトリ構成

- `nekoui-rs/`: Rust backend workspace
- `nekoui-gui/`: React Router ベースのフロントエンドスキャフォールド
...
`nekoui-rs/` は Cargo workspace で、主なクレートは次の通りです。
...
- `just`
- Docker（Qdrant を使う場合）
- Node.js（任意、`nekoui-gui/` を触る場合）

### バックエンド

以下のコマンドは `nekoui-rs/` で実行します。
...
- システムプロンプトは `.config/INSTRUCTION.md` に置きます。なければデフォルトのプロンプトが使われます
...
- 環境変数は `DISCORD_AGENT_TOKEN`, `NEKOUI_API_KEY`, `NEKOUI_PROVIDER`, `NEKOUI_MODEL`, `NEKOUI_BASE_URL`, `NEKOUI_GUILD_ID` です

```bash
DISCORD_AGENT_TOKEN=... NEKOUI_API_KEY=... NEKOUI_PROVIDER=openai NEKOUI_MODEL=gpt-4o just neko start --skip-setup
```

### ログ

- `LOG_LEVEL` は `nekoui-rs/.env` で設定します
- 例: `LOG_LEVEL=info` や `LOG_LEVEL=nekoui-agent=debug,nekoui-cli=debug`
...
```bash
cargo run --bin nekoui-cli -- start
```
...
- 実行時は OpenAI 互換 API を使うため、カスタム URL には互換エンドポイントを持つ必要があります
...
| `/clear` | slash / `w!clear` | 現在のセッションをクリアします。短期メモリはバックグラウンドで mid-term に反映します。 |
| `/history` | slash only | 直近の会話履歴を表示します。長い履歴は Discord の文字数制限に達する可能性があります。 |
